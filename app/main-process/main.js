...
ipc.on("save-data", (sender, data) => {
    console.log("Save Data!!!")
    const { BrowserWindow } = require('electron');
    const parsedData = JSON.parse(data)
    const allWindows = BrowserWindow.getAllWindows();

    // Send a message to each window
    allWindows.forEach(window => {
      window.webContents.send('save-data', parsedData.character);
    });
})

ipc.on("save-story", (sender, data) => {
    ...
    // Send a message to each window
    allWindows.forEach(window => {
      window.webContents.send('save-story', parsedData);
    });
})
