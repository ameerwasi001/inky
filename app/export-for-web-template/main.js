const loadFont = (fontPath, cb) => {
    const realPath = fontPath
    const fontName = `font-${fontPath.split("/")[fontPath.split("/").length - 1]}`
    const font = new FontFace(fontName, `url("${realPath}")`)
    font.load().then(loadedFace => {
        document.fonts.add(loadedFace)
        cb(fontName, loadedFace)
    })
}
