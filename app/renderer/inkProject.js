        ...
        if(save && save.filePath){
            ...
            if( targetSavePath ) {
                ...
                // Web export
                    ...
                    const platformSep = process.platform.includes("window") ? "\\" : "/"
                    const directoryPath = this.mainInk.absolutePath().split("\\").join("/").split("/").slice(0, -1).join(platformSep)
                    console.log("DPATH", directoryPath)
...
InkProject.prototype.buildForWeb = function(jsonFilePath, targetDirectory, inkoPath) {
    console.log("Building for the web...", targetDirectory)
    ...
    fsExtra.copy(inkoPath, targetDirectory, { overwrite: true }, (err) => {
        if (err) {
